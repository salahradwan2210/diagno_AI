...
import joblib as jb
model = jb.load('./MultinomialNB1.pkl')
...
def get_disease_details(disease_name):
    ...
    description_file = os.path.join(settings.BASE_DIR, 'myapp/dataset/description.csv')
    diet_file = os.path.join(settings.BASE_DIR, 'myapp/dataset/diets.csv')
    precautions_file = os.path.join(settings.BASE_DIR, 'myapp/dataset/precautions_df.csv')
    medications_file = os.path.join(settings.BASE_DIR, 'myapp/dataset/medications.csv')
    workouts_file = os.path.join(settings.BASE_DIR, 'myapp/dataset/workout_df.csv')

    
    disease_details = {
        'description': 'Description not found',
        'precautions': 'Precautions information not found',
        'medications': 'Medications information not found',
        'workouts': 'Workouts information not found',
        'diet': 'Diet information not found',
    }
        ...
        with open(description_file, mode='r') as file:
            ...
            for row in reader:
                if row['Disease'].strip().lower() == disease_name.strip().lower():
                    disease_details['description'] = row['Description']
                    break

        # Read precautions
        with open(precautions_file, mode='r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                if row['Disease'].strip().lower() == disease_name.strip().lower():
                    disease_details['precautions'] = ", ".join([
                        row['Precaution_1'], 
                        row['Precaution_2'], 
                        row['Precaution_3'], 
                        row['Precaution_4']
         ...
         # Read medications_file
        with open(medications_file, mode='r') as file:
            reader = csv.DictReader(file)
            for row in reader:
               if row['Disease'].strip().lower() == disease_name.strip().lower():
                  disease_details['medications'] = row['Medication']
                  break
        # Read diet
        with open(diet_file, mode='r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                if row['Disease'].strip().lower() == disease_name.strip().lower():
                    disease_details['diet'] = row['Diet']
                    break
                
         # Read workouts_file
        with open(workouts_file, mode='r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                if row['disease'].strip().lower() == disease_name.strip().lower():
                    disease_details['workout'] = row['workout']
                    break
  ...
  if request.method == 'GET':
     ...
     return render(request,'patient/checkdisease/checkdisease.html', {"list2":alphabaticsymptomslist})
      ...
      return JsonResponse({
            ...
            'disease_diet': disease_details['diet'],
            'disease_precautions': disease_details['precautions'],
            'disease_medications': disease_details['medications'],
            'disease_workouts': disease_details['workout'],
...
def make_consultation(request, doctorusername):
    if request.method == 'POST':
        ...
        try:
            # Check if a consultation with the same diseaseinfo already exists
            existing_consultation = consultation.objects.get(diseaseinfo=diseaseinfo_obj)
            # If found, redirect to the existing consultation view
            request.session['consultation_id'] = existing_consultation.id
            print("Consultation record already exists, redirecting to the existing consultation...")
            return redirect('consultationview', existing_consultation.id)
        
        except consultation.DoesNotExist:
            # If no existing consultation, create a new one
